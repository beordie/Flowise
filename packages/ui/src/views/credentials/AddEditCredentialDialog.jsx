    ...
    const setOAuth2 = async () => {
        try {
            ...
            if (authResponse.data && authResponse.data.success && authResponse.data.authorizationUrl) {
                ...
                if (!authWindow) {
                    throw new Error('无法打开授权窗口。请检查是否阻止了弹出窗口。')
                }
                ...
                const handleMessage = (event) => {
                    ...
                    if (event.data && (event.data.type === 'OAUTH2_SUCCESS' || event.data.type === 'OAUTH2_ERROR')) {
                        ...
                        if (event.data.type === 'OAUTH2_SUCCESS') {
                            ...
                            enqueueSnackbar({
                                message: event.data.message || 'OAuth2授权失败',
                                options: {
                                    key: new Date().getTime() + Math.random(),
                                    variant: 'error',
                                    persist: true,
                ...
                throw new Error('授权端点响应无效')
            }
        } catch (error) {
            console.error('OAuth2 authorization error:', error)
            if (setError) setError(error)
            enqueueSnackbar({
                message: `OAuth2授权失败: ${error.response?.data?.message || error.message || '未知错误'}`,
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
        ...
        >
            ...
            <DialogContent>
                {shared && (
                    ...
                    >
                        ...
                        >
                            ...
                            <span style={{ color: 'white', marginLeft: 10, fontWeight: 400 }}>无法编辑共享凭证。</span>
                        </div>
                    </div>
                )}
                ...
                {!shared && componentCredential && componentCredential.name && componentCredential.name.includes('OAuth2') && (
                    <Box sx={{ p: 2 }}>
                        <Stack sx={{ position: 'relative' }} direction='row'>
                            <Typography variant='overline'>OAuth重定向URL</Typography>
                        </Stack>
                        <OutlinedInput
                            id='oauthRedirectUrl'
                            type='string'
